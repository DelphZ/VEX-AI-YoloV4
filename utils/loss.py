...
def build_targets(p, targets, model):
    ...
    for i, jj in enumerate(model.module.yolo_layers if multi_gpu else model.yolo_layers):
        ...
        if nt:
            ...
            at = torch.arange(na, device=targets.device).view(na, 1).repeat(1, nt) # ensure at is created upon GPU
        ...
        indices.append((b, a, gj.clamp_(0, (gain[3] - 1.0).to(torch.int64)), gi.clamp_(0, (gain[2] - 1.0).to(torch.int64))))  # ensure data type matched
