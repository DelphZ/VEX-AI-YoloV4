...
This is a PyTorch implementation of YOLOv4 designed for VEX-AI, optimized to run across different platforms. The goal of this project is to make setup simple enough that even young students with only basic programming knowledge can train their own vision models and use them in VEX-AI competitions. It currently supports running on older CUDA 4.6 versions with the Jetson Nano, as well as newer PyTorch versions on your computers. The project is fully adapted for VEX Robotics and also includes support for depth camera models with Astra Pro Plus. 
...
For trainning with a limited GPU (e.g: GPU in your Jetson Nano B01):

```
python3 ./train.py --device 0 --batch-size 2 --img 512 512 --data ./_vex.yaml --cfg ./cfg/yolov4-tiny.cfg --name yolov4-vex-detect --epochs 300 --weights ./yolov4.conv.137 --workers 2 --hyp ./data/hyp.scratch.s.yaml
```

For trainning with a better GPU:
```
python3 ./train.py --device 0 --batch-size 8 --img 640 640 --data ./_vex.yaml --cfg ./cfg/yolov4-tiny.cfg --name yolov4-vex-detect --epochs 300 --weights ./yolov4.conv.137 --workers 2 --hyp ./data/hyp.scratch.s.yaml
```

The trainning process will always record the best performance model using the valid set provided, so if you find the model performance has no progress, just CTRL+C with hard stop is fine...

## Testing

```
python test.py --img 640 --conf 0.001 --batch 8 --device 0 --data coco.yaml --cfg ./cfg/yolov4-tiny.cfg --weights your_best_weight_file.pt
```
